...
/**
 * Multi-page report (U3, D3). Renders a persisted multi-page audit from the
 * master `Audit.result` light shape (`multiPageResultSchema`): the aggregate
 * hero (ScoreHero over the light aggregate — its shape is the summary shape)
 * plus one row per audited page: url + GEO Score + severity band + duration.
 *
          ...
          {result.pages.map((page) => (
            ...
            >
              ...
              <span className="flex items-center gap-3 text-sm text-text-secondary">
                ...
                <span>{formatDurationMs(page.durationMs)}</span>
              </span>
            </li>
          ))}
        </ul>
