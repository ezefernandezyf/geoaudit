...
Compute a weighted composite GEO Score (0-100) from the five available engine scores using the Sprint 1 renormalized weight configuration. Assign a severity band label and tag per-engine findings with severity levels. Embed a `scoringModelVersion` field for traceable weight evolution across sprints.

## Requirements

| # | Requirement | Strength | Summary |
|---|-------------|----------|---------|
| RGS-1 | Renormalized weights | MUST | Use Sprint 1 weights: citability 31.25%, E-E-A-T 25%, Technical 18.75%, Schema 12.5%, Platform 12.5% |
...
| RGS-5 | Severity band assignment | MUST | Map rounded score to band: 90-100 Excellent, 75-89 Good, 60-74 Fair, 40-59 Poor, 0-39 Critical |
...
| RGS-7 | scoringModelVersion | MUST | Output MUST include `scoringModelVersion: "1.0.0"` and a weights config object documenting the renormalization |
| RGS-8 | Brand Authority note | MUST | Output MUST note that Brand Authority (20% in brief §8.1) was renormalized out in v1.0.0 and will re-enter with a future model version |
...
The calculator MUST use the Sprint 1 renormalized weight distribution.
...
- WHEN the composite is computed
- THEN the result reflects weights: (60×0.3125 + 90×0.25 + 50×0.1875 + 100×0.125 + 40×0.125) = 68.125 → 68
...
- THEN the output contains `scoringModelVersion: "1.0.0"`
...
|-------------|-----------|----------|
| RGS-1 | All engines score 80, Uneven scores with weights | Covered |
...
| RGS-5 | Score 92 Excellent, Score 74 Fair, Score 39 Critical, Score 100 cap | Covered |
