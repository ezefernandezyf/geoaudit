...
- **landing**: `brand.ts` (+`knowsAbout`/`contactPoint`/`email` verificables), `copy.ts` (`LANDING_COPY.faq`, `datePublished`, byline), `page.tsx` (Organization enriquecida, FAQ sección + FAQPage JSON-LD, fechas/byline, alt).
...
| `src/app/page.tsx` | Modify | Organization enriquecida, FAQ visible, FAQPage JSON-LD, fechas/byline/alt |
...
- **Nuevos**: `schema/index.test.ts` (toContractResult mapea score, RSC-14); `page.test.tsx` (FAQ + FAQPage JSON-LD, Organization recommended props LND-9, fechas/byline/alt LND-13).
...
2. **FAQPage vs engine**: emitir FAQPage dispara `deprecated_faqpage` (docking ~5 pts schema) — neto positivo en citabilidad, posible baja en schema. Verificar en re-auditoría.
...
4. Landing: JSON-LD enriquecido, FAQ + FAQPage, fechas/byline/alt.
